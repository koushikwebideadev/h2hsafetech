...
@section('content')
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Add Partner Logo</h1>
            <p class="text-slate-500 text-sm">Upload a logo for the homepage rolling banner.</p>
        </div>
    ...
    <div class="glass-card max-w-4xl">
        ...
        <div class="p-6">
            <form action="{{ route('admin.partner-logos.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="mb-6">
                    <label for="image" class="block text-sm font-semibold text-slate-700 mb-2">Logo Image <span class="text-red-500">*</span></label>
                    ...
                    <p class="text-xs text-slate-400 mt-1">Recommended: PNG/SVG with transparent background, max height ~50px</p>
                    @error('image')
                ...
                <div class="mb-6" x-show="linkType === 'page'" x-cloak>
                    ...
                    @if ($pages->isEmpty())
                        <p class="text-xs text-amber-600 mt-1">No custom pages yet. <a href="{{ route('admin.custom-pages.create') }}" class="underline">Create one first</a>.</p>
                    @endif
                    @error('page_id')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
