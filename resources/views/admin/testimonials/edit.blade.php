...
@section('content')
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 text-sm">Update the testimonial from {{ $testimonial->name }}.</p>
        </div>
            ...
            <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST"
                ...
                <div class="mb-6">
                    ...
                    <textarea name="testimonial" id="testimonial" rows="4" required
                        class="editor w-full rounded-xl border-slate-200 focus:border-violet-500 focus:ring-violet-500 transition-all text-sm @error('testimonial') border-red-500 @enderror">{{ old('testimonial', $testimonial->testimonial) }}</textarea>
                    @error('testimonial')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    <label for="image" class="block text-sm font-semibold text-slate-700 mb-2">Image</label>
                    ...
                    <p class="mt-2 text-xs text-slate-400 italic">Leave blank to keep the current image.</p>
                    @error('image')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                    ...
                    @if($testimonial->image)
                        <div class="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-xs font-bold text-slate-700">Current Image</p>
                                <p class="text-xs text-slate-400">Successfully uploaded</p>
                            </div>
                        </div>
                    @endif
                </div>
