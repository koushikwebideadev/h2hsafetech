...
@section('title', 'Edit Feature Category')

@section('content')
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Edit Feature Category</h1>
            <p class="text-slate-500 text-sm">Update category information and items.</p>
        </div>
    ...
    <div class="max-w-6xl">
        <form action="{{ route('admin.feature-categories.update', $featureCategory) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="glass-card p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" placeholder="e.g., Society Share"
                            value="{{ old('title', $featureCategory->title) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="tab_order" value="{{ old('tab_order', $featureCategory->tab_order) }}"
                            class="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-violet-100"
                            required>
                    </div>
                </div>
                ...
                <div class="flex items-center">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $featureCategory->is_active) ? 'checked' : '' }} class="w-4 h-4 text-violet-600 rounded">
            ...
            <div class="glass-card p-6">
                ...
                <div id="feature-items-container" class="space-y-3">
                    @foreach($featureCategory->items as $index => $item)
                        <div class="bg-slate-50 p-4 rounded-lg feature-item space-y-3">
                            <div class="grid grid-cols-12 gap-3">
                                <input type="text" name="items[{{ $index }}][icon]" placeholder="Icon class"
                                    value="{{ $item->icon }}"
                                ...
                                <input type="text" name="items[{{ $index }}][title]" placeholder="Feature title"
                                ...
                                <input type="text" name="items[{{ $index }}][description]" placeholder="Short Description"
                            ...
                            <div>
                                <label class="block text-xs font-bold text-slate-600 mb-1">Long Description (for Read More
                                    popup)</label>
                                <textarea name="items[{{ $index }}][long_description]"
                                    ...
                                    rows="4"
                                    placeholder="Enter detailed description...">{{ $item->long_description }}</textarea>
                            </div>
    ...
    <script>
        let itemIndex = {{ $featureCategory->items->count() }};
        let editorInstances = {};
        ...
        // Initialize CKEditor for existing items
        document.addEventListener('DOMContentLoaded', function () {
            document.querySelectorAll('.ckeditor-editor').forEach((textarea, index) => {
                if (!textarea.id) {
                    textarea.id = `editor-existing-${index}`;
                }
